...
# Preencha aqui ou execute em modo interativo.
# ==============================================================================

SMTP_HOST="${SMTP_HOST:-smtp.example.com}"
SMTP_PORT="${SMTP_PORT:-25}"
SMTP_USER="${SMTP_USER:-naoresponda@example.com}"
SMTP_PASS="${SMTP_PASS:-}"
MAIL_FROM="${MAIL_FROM:-naoresponda@example.com}"
TEST_TO="${TEST_TO:-destino@example.com}"

# starttls = porta 25/587 com STARTTLS
# wrapper  = porta 465 com SSL/TLS direto
TLS_MODE="${TLS_MODE:-starttls}"
TLS_PROTOCOL="${TLS_PROTOCOL:-TLSv1.2}"

# Segurança: escutar só localmente evita relay aberto.
...
ask(){
  ...
  if [[ "$secret" == "yes" ]]; then
    read -r -s -p "$label [oculto]: " value; echo
  else
...
install_pkgs(){
  ...
  if command -v dnf >/dev/null 2>&1; then
    ...
    dnf install -y postfix mailx cyrus-sasl cyrus-sasl-lib cyrus-sasl-plain swaks nc openssl
  elif command -v yum >/dev/null 2>&1; then
    yum install -y postfix s-nail cyrus-sasl cyrus-sasl-lib cyrus-sasl-plain swaks nmap-ncat openssl || \
    yum install -y postfix mailx cyrus-sasl cyrus-sasl-lib cyrus-sasl-plain swaks nc openssl
  else
...
free_gid(){
  awk -F: 'BEGIN{for(i=980;i<=1100;i++) used[i]=0} {used[$3]=1} END{for(i=980;i<=1100;i++) if(!used[i]){print i; exit}}' /etc/group
}

fix_postdrop_group(){
  log "Validando grupo postdrop..."
  if ! getent group postdrop >/dev/null 2>&1; then
    local gid; gid="$(free_gid)"
    [[ -n "$gid" ]] || fail "Sem GID livre entre 980 e 1100."
    groupadd -r -g "$gid" postdrop || echo "postdrop:x:$gid:" >> /etc/group
  fi

  local pg rg
  pg="$(getent group postdrop | awk -F: '{print $3}')"
  rg="$(getent group render 2>/dev/null | awk -F: '{print $3}' || true)"
  if [[ -n "$rg" && "$pg" == "$rg" ]]; then
    warn "postdrop e render usam o mesmo GID ($pg). Corrigindo."
    cp -p /etc/group "$BACKUP_DIR/group.before-postdrop-fix.bkp"
    local ng; ng="$(free_gid)"
    awk -F: '$1!="postdrop"{print}' /etc/group > /etc/group.tmp.postfix
    mv /etc/group.tmp.postfix /etc/group
    echo "postdrop:x:$ng:" >> /etc/group
  fi
}
...
EOF
  if [[ "$SMTP_PORT" != "465" ]]; then
    echo "[$SMTP_HOST]:465 $SMTP_USER:$SMTP_PASS" >> /etc/postfix/sasl_passwd
  fi
...
validate(){
  ...
  log "Validação TLS:"
  if [[ "$TLS_MODE" == "wrapper" ]]; then
    timeout 12 openssl s_client -connect "$SMTP_HOST:$SMTP_PORT" -tls1_2 -servername "$SMTP_HOST" </dev/null || warn "openssl wrapper falhou."
  else
    timeout 12 openssl s_client -starttls smtp -connect "$SMTP_HOST:$SMTP_PORT" -tls1_2 -servername "$SMTP_HOST" </dev/null || warn "openssl STARTTLS falhou."
  fi
}

test_swaks(){
  command -v swaks >/dev/null 2>&1 || { warn "swaks não encontrado."; return 0; }
  local helo; helo="$(hostname -f 2>/dev/null || hostname)"
  log "Teste swaks..."
  if [[ "$TLS_MODE" == "wrapper" ]]; then
    swaks --to "$TEST_TO" --from "$MAIL_FROM" --server "$SMTP_HOST" --port "$SMTP_PORT" --helo "$helo" --tls-on-connect --tls-protocol TLSv1_2 --auth LOGIN --auth-user "$SMTP_USER" --auth-password "$SMTP_PASS" --header "Subject: Teste swaks $(hostname)" --body "Teste swaks $(date)"
  else
    swaks --to "$TEST_TO" --from "$MAIL_FROM" --server "$SMTP_HOST" --port "$SMTP_PORT" --helo "$helo" --tls --tls-protocol TLSv1_2 --auth LOGIN --auth-user "$SMTP_USER" --auth-password "$SMTP_PASS" --header "Subject: Teste swaks $(hostname)" --body "Teste swaks $(date)"
  fi
}
...
test_local(){
  ...
  local cmd
  if command -v s-nail >/dev/null 2>&1; then cmd=s-nail
  elif command -v mailx >/dev/null 2>&1; then cmd=mailx
  else warn "s-nail/mailx não encontrado."; return 0
  fi
